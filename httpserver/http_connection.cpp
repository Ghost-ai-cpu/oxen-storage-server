...
static constexpr auto WORKTIPS_FILE_SERVER_TARGET_HEADER =
    "X-Worktips-File-Server-Target";
static constexpr auto WORKTIPS_FILE_SERVER_VERB_HEADER = "X-Worktips-File-Server-Verb";
static constexpr auto WORKTIPS_FILE_SERVER_HEADERS_HEADER =
    "X-Worktips-File-Server-Headers";
...
// ======================== Worktipsd Client ========================
WorktipsdClient::WorktipsdClient(boost::asio::io_context& ioc, std::string ip,
                         uint16_t port)
...
static bool validateHexKey(const std::string& key,
                           const size_t key_length = worktips::KEY_LENGTH) {
...
WorktipsdClient::wait_for_privkey() {
    // fetch SN private key from worktipsd; do this synchronously because we can't
    ...
    key_fetch = [&](worktips::sn_response_t res) {
        try {
            ...
            WORKTIPS_LOG(critical,
                     "Error retrieving SN privkey from worktipsd @ {}:{}: {}.  Is "
                     "worktipsd running?  Retrying in 5s",
...
void connection_t::on_handshake(boost::system::error_code ec) {
    ...
    if (ec) {
        WORKTIPS_LOG(debug, "ssl handshake failed: ec: {} ({})", ec.value(),
                 ec.message());
        this->clean_up();
        deadline_.cancel();
        return;
...
void connection_t::process_storage_test_req(uint64_t height,
    ...
    if (status == MessageTestStatus::SUCCESS) {
        ...
        WORKTIPS_LOG(debug, "Failed storage test, tried {} times.",
                 repetition_count_);
        nlohmann::json json_res;
...
void connection_t::process_onion_req() {
    ...
    WORKTIPS_LOG(debug, "Processing an onion request");
    ...
    auto on_response = [wself = std::weak_ptr<connection_t>{shared_from_this()}](worktips::Response res) {
        WORKTIPS_LOG(debug, "Got an onion response as guard node");

        auto self = wself.lock();
        if (!self) {
            WORKTIPS_LOG(debug, "Connection is no longer valid, dropping onion response");
            return;
        }
...
void connection_t::process_proxy_req() {
    ...
    WORKTIPS_LOG(debug, "[{}] Processing proxy request: we are first hop", req_idx);
    ...
    if (!parse_header(WORKTIPS_SENDER_KEY_HEADER, WORKTIPS_TARGET_SNODE_KEY)) {
    ...
    WORKTIPS_LOG(debug, "[{}] Destination: {}", req_idx, target_snode_key);
    ...
    auto on_proxy_response = [wself = std::weak_ptr<connection_t>{shared_from_this()}, req_idx](
        ...
        WORKTIPS_LOG(debug, "on proxy response: {}",
                 success ? "success" : "failure");

        auto self = wself.lock();
        if (!self) {
            WORKTIPS_LOG(debug, "Connection is no longer valid, dropping proxy response");
            return;
        }

        if (!success) {
            WORKTIPS_LOG(debug, "Proxy response FAILED (timeout), idx: {}", req_idx);
            self->response_.result(http::status::gateway_timeout);
        } else if (data.size() == 2) {
            WORKTIPS_LOG(debug, "Proxy respose with status, idx: {}", req_idx);
            ...
            WORKTIPS_LOG(debug, "Proxy response FAILED (wrong data size), idx: {}", req_idx);
            self->response_.result(http::status::internal_server_error);
        } else {
            WORKTIPS_LOG(debug, "PROXY RESPONSE OK, idx: {}", req_idx);
    ...
    if (!sn) {
        WORKTIPS_LOG(debug, "Could not find target snode for proxy: {}",
                 target_snode_key);
        on_proxy_response(false, {});
        return;
    }

    WORKTIPS_LOG(debug, "Target Snode: {}", target_snode_key);

    // Send this request to SN over either HTTP or WORKTIPSMQ
    auto sn_req =
        ss_client::Request{req.body(), {{WORKTIPS_SENDER_KEY_HEADER, sender_key}}};

    WORKTIPS_LOG(debug, "About to send a proxy exit requst, idx: {}", req_counter);
    req_counter += 1;
...
void connection_t::process_file_proxy_req() {
    ...
    auto cb = [wself = std::weak_ptr<connection_t>{shared_from_this()}](sn_response_t res) {
        ...
        if (res.raw_response) {
            ...
            WORKTIPS_LOG(trace, "Response: {}", self->response_);
        } else {
...
void connection_t::process_swarm_req(boost::string_view target) {
    ...
    if (target == "/swarms/push_batch/v1") {
        ...
        response_.result(http::status::bad_request);
        WORKTIPS_LOG(trace, "Got storage test request");
        ...
        WORKTIPS_LOG(trace, "Received ping_test");
...
void connection_t::set_response(const Response& res) {
    ...
    case ContentType::json:
        ...
        break;
    default:
        WORKTIPS_LOG(critical, "Unrecognized content type");
    }
...
void connection_t::process_request() {
    ...
    WORKTIPS_LOG(debug, "connection_t::process_request");
    ...
    if (is_swarm_req) {
        WORKTIPS_LOG(debug, "Processing a swarm request: {}", target);
    }
    ...
    case http::verb::post: {
        ...
        if (!service_node_.snode_ready(reason)) {
            WORKTIPS_LOG(debug,
                     ...
                     reason);
            WORKTIPS_LOG(debug, "Would send 503 error (2)");
...
void connection_t::write_response() {
    ...
    WORKTIPS_LOG(trace, "write response, {} bytes", response_.body().size());
...
void connection_t::process_client_req_rate_limited() {
    ...
    WORKTIPS_LOG(trace, "process_client_req_rate_limited");
    ...
    if (req.find(WORKTIPS_LONG_POLL_HEADER) != req.end()) {
        header_[WORKTIPS_LONG_POLL_HEADER] =
            req.at(WORKTIPS_LONG_POLL_HEADER).to_string();
    }
    ...
    if (lp_requested) {
        WORKTIPS_LOG(debug, "Received a long-polling request");
        ...
        delay_timer->async_wait([self = shared_from_this(), delay_timer, plaintext = std::move(plain_text)](const error_code& ec) {
            ...
            self->request_handler_.process_client_req(plaintext, [wself = std::weak_ptr<connection_t>{self}](worktips::Response res) {

                auto self = wself.lock();
                if (!self) {
                    WORKTIPS_LOG(debug, "Connection is no longer valid, dropping response");
                    return;
                }

                WORKTIPS_LOG(debug, "Respond to a long-polling client");
        ...
        request_handler_.process_client_req(
            plain_text,
            [wself = std::weak_ptr<connection_t>{shared_from_this()}](worktips::Response res) {
                ...
                WORKTIPS_LOG(debug, "Respond to a non-long polling client");
                self->set_response(res);
                self->write_response();
            });
    }
...
void connection_t::register_deadline() {
    ...
    deadline_.async_wait([self = shared_from_this()](error_code ec) {
        ...
        WORKTIPS_LOG(debug, "[{}] Closing [connection_t] socket due to timeout", self->conn_idx);
        self->clean_up();
    });
}
...
void HttpClientSession::clean_up() {
    ...
    if (ec) {
        WORKTIPS_LOG(error, "Closing socket {} failed [{}: {}]", sockfd, ec.value(),
                 ec.message());
    } else {
