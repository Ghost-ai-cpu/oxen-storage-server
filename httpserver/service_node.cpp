...
parse_swarm_update(const std::shared_ptr<std::string>& response_body) {
    ...
    WORKTIPS_LOG(trace, "swarm repsonse: <{}>", *response_body);
    ...
    try {
        ...
        for (const auto& sn_json : service_node_states) {
            ...
            if (pubkey_x25519_hex.empty()) {
                WORKTIPS_LOG(warn, "pubkey_x25519_hex is missing from sn info");
                continue;
            }

            // worktipsdKeyFromHex works for pub keys too
            const public_key_t pubkey_x25519 =
                worktipsdKeyFromHex(pubkey_x25519_hex);
            ...
            if (pubkey_ed25519.empty()) {
                WORKTIPS_LOG(warn, "pubkey_ed25519 is missing from sn info");
                continue;
            }
        ...
        worktipsd_client_.make_custom_worktipsd_request(
            ...
            [this, seed_node, req_counter,
             node_count = seed_nodes.size()](const sn_response_t&& res) {
                if (res.error_code == SNodeError::NO_ERROR) {
                    WORKTIPS_LOG(info, "Parsing response from seed {}",
                             seed_node.first);
                    try {
                        ...
                        WORKTIPS_LOG(info, "Bootstrapped from {}", seed_node.first);
...
void ServiceNode::send_onion_to_sn(const sn_record_t& sn,
    ...
    lmq_server_->request(
        ...
        worktipsmq::send_option::request_timeout{10s}, eph_key, payload);
}
...
void ServiceNode::send_to_sn(const sn_record_t& sn, ss_client::ReqMethod method,
    ...
    case ss_client::ReqMethod::DATA: {
        WORKTIPS_LOG(debug, "Sending sn.data request to {}",
    ...
    case ss_client::ReqMethod::PROXY_EXIT: {
        auto client_key = req.headers.find(WORKTIPS_SENDER_KEY_HEADER);

        // I could just always assume that we are passing the right
        // parameters...
        if (client_key != req.headers.end()) {
            WORKTIPS_LOG(debug, "Sending sn.proxy_exit request to {}",
                     util::as_hex(sn.pubkey_x25519_bin()));
            lmq_server_->request(sn.pubkey_x25519_bin(), "sn.proxy_exit",
                                       std::move(cb), client_key->second,
                                       req.body);
        } else {
            WORKTIPS_LOG(debug, "Developer error: no {} passed in headers",
                     WORKTIPS_SENDER_KEY_HEADER);
            // TODO: call cb?
            assert(false);
        }
        break;
    ...
    case ss_client::ReqMethod::ONION_REQUEST: {
        // Onion reqeusts always use worktipsmq, so they use it
        // directly, no need for the "send_to_sn" abstraction
        WORKTIPS_LOG(error, "Onion requests should not use this interface");
        assert(false);
        break;
    }
    }
...
void ServiceNode::relay_data_reliable(const std::string& blob,
    ...
    auto reply_callback = [](bool success, std::vector<std::string> data) {
        if (!success) {
            WORKTIPS_LOG(error, "Failed to send batch data: time-out");
        }
    };

    WORKTIPS_LOG(debug, "Relaying data to: {}", sn);
...
void ServiceNode::on_swarm_update(block_update_t&& bu) {
    ...
    if (this->hardfork_ != bu.hardfork) {
        WORKTIPS_LOG(debug, "New hardfork: {}", bu.hardfork);
    ...
    if (this->status_ != status) {
        WORKTIPS_LOG(info, "Node status updated: {}", status);
        this->status_ = status;
    }
...
void ServiceNode::relay_buffered_messages() {
    ...
    WORKTIPS_LOG(debug, "Relaying {} messages from buffer to {} nodes",
            ...
            if (res.error_code == SNodeError::NO_ERROR) {
                try {
                    ...
                    if (!got_first_response) {
                        WORKTIPS_LOG(
                            info,
                            "Got initial swarm information from local Worktipsd");
                        got_first_response = true;
#ifndef INTEGRATION_TEST
                        // Only bootstrap (apply ips) once we have at least
                        // some entries for snodes from worktipsd
                        this->bootstrap_data();
#endif
                    }
...
void ServiceNode::update_last_ping(ReachType type) {
    ...
    switch (type) {
        ...
        default:
            WORKTIPS_LOG(error, "Connection type not supported");
            assert(false);
            break;
    }
}
...
void ServiceNode::ping_peers_tick() {
    ...
    if (this->status_ == SnodeStatus::UNSTAKED || this->status_ == SnodeStatus::UNKNOWN) {
        WORKTIPS_LOG(debug, "Skipping this round of peer testing (unstaked)");
        return;
    }
    ...
    if (this->status_ == SnodeStatus::DECOMMISSIONED) {
        WORKTIPS_LOG(debug, "Skipping this round of peer testing (decommissioned)");
        return;
    }
    ...
    if (random_node) {
        ...
        if (random_node == our_address_) {
            WORKTIPS_LOG(trace, "Would test our own node, skipping");
        } else {
            WORKTIPS_LOG(trace, "Selected random node for testing: {}",
                     (*random_node).pub_key_hex());
            test_reachability(*random_node);
        }
    } else {
        WORKTIPS_LOG(trace, "No nodes to test for reachability");
    }
...
void ServiceNode::test_reachability(const sn_record_t& sn) {
    ...
    WORKTIPS_LOG(debug, "Testing node for reachability over HTTP: {}", sn);

    auto callback = [this, sn](sn_response_t&& res) {
        WORKTIPS_LOG(debug, "Got response for HTTP peer test for: {}", sn);
    ...
    WORKTIPS_LOG(debug, "Testing node for reachability over LMQ: {}", sn);

    // test lmq port:
    lmq_server_->request(
        sn.pubkey_x25519_bin(), "sn.onion_req",
        [this, sn](bool success, const auto&) {
            WORKTIPS_LOG(debug, "Got success={} testing response from {}", success,
        ...
        worktipsmq::send_option::outgoing{});
}
...
void ServiceNode::process_reach_test_result(const sn_pub_key_t& pk,
    ...
    if (success) {
        ...
        WORKTIPS_LOG(trace, "Recording node as unreachable");
...
bool ServiceNode::derive_tester_testee(uint64_t blk_height, sn_record_t& tester,
    ...
    if (members.size() < 2) {
        WORKTIPS_LOG(trace, "Could not initiate peer test: swarm too small");
        return false;
    }
    ...
    if (blk_height == block_height_) {
        ...
        WORKTIPS_LOG(trace, "got storage test request for an older block: {}/{}",
        ...
        if (it != block_hashes_cache_.end()) {
            ...
            WORKTIPS_LOG(trace, "Could not find hash for a given block height");
...
void ServiceNode::initiate_peer_test() {
    ...
    WORKTIPS_LOG(trace, "For height {}; tester: {} testee: {}", test_height, tester,
             testee);
...
void ServiceNode::relay_messages(const std::vector<Message>& messages,
    ...
    WORKTIPS_LOG(debug, "Relayed messages:");
    for (auto msg : batches) {
        WORKTIPS_LOG(debug, "    {}", msg);
    }
    WORKTIPS_LOG(debug, "To Snodes:");
    for (auto sn : snodes) {
        WORKTIPS_LOG(debug, "    {}", sn);
    }

    WORKTIPS_LOG(debug, "Serialised batches: {}", batches.size());
